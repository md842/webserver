...
integration_test (){
  ...
  then
    ...
    $2 $3 < $4 > $OUTPUT_FILE & # Run netcat command as background process, redirecting stdin to $4, stdout to output
  fi

  sleep 0.1 # Small delay for server processing/output to become available
  diff $OUTPUT_FILE $1 # Compare actual output to expected output
  ...
  then
    if [ $2 = "curl" ] # If command is curl
      then
        printf "Test $NUM FAIL: $2 $3\n" >> $RESULT_FILE # Print info about curl
      else
        printf "Test $NUM FAIL: $2 $3 < $4\n" >> $RESULT_FILE # Print info about netcat
    fi
...
rm $RESULT_FILE # Remove previous test results
touch $RESULT_FILE # Create new test results file
NUM=0 # Initialize test number counter
$SERVER_BINARY $CONFIG_FILE & # Ending with & runs command as background process
